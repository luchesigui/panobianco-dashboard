...
import { runSyncJob } from "@/lib/services/receivables-sync";
...
export async function GET(req: Request) {
	try {
		...
		const revenueCenters = await crm.getRevenueCenter();
		const revenueCentersMap = Object.fromEntries(
			revenueCenters.map((rc) => [String(rc.idRevenueCenter), rc.description]),
		);
		...
		const { data: newJob } = await supabase
			...
			.insert({
				...
				revenue_centers: revenueCentersMap,
			})
			.select("id")
			.single();
