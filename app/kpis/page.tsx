...
import { ForecastDeepDive } from "./ForecastDeepDive";
...
const sections: Array<{
	...
	},
	{
		...
		theme: {
			themeSection: styles.themeSectionPrevisao,
			iconShort: "P",
			iconVariant: styles.themeIconPrevisao,
			badgeVariant: styles.badgePrevisao,
			insightHeader: styles.headerPrevisao,
		},
		cards: [],
	},
	{
...
function RetentionKpiCards({ data }: { data: KpiPageData }) {
	...
	const monthShort = (
		data.currentPeriodLabel.split("/")[0] ?? "mês"
	).toLowerCase();
			...
			<header className={styles.header}>
				<div className={styles.headerTop}>
					...
					<div className={styles.periodStrip}>
						<span>Período:</span>
						<strong>{data.currentMonthLabel}</strong>
					</div>
				</div>
			</header>
				...
				return (
					...
					>
						<div className={styles.themeHeader}>
							...
							<h2>{section.title}</h2>
							<span
								className={`${styles.themeBadge} ${section.theme.badgeVariant}`}
							>
								{section.id === "roi"
									? "Desde Jul/24"
						...
						) : section.id === "forecast" ? null : section.id === "roi" ? (
							<div className={styles.kpiGrid}>
									...
									if (section.id === "sales_marketing") {
										...
										const deltaOpts =
											...
											typeof metaObj.delta_pct_display === "number"
												? {
														deltaPctDisplay:
															metaObj.delta_pct_display as number,
													}
												: undefined;
										...
										if (
											card.key === "instagram_total_reach" &&
											current != null
										) {
						...
						{section.id === "sales_marketing" ? (
							...
							<>
								...
								{data.nextMonthForecast.hasData ? (
									<ForecastDeepDive forecast={data.nextMonthForecast} />
								) : (
									<p className={styles.subtitle}>
				...
				<section className={styles.sectionPlain}>
					<div className={styles.feature}>
						...
						<div className={styles.impactList}>
							{Object.entries(data.featureOfMonth.impact).map(
								([key, value]) => (
									<span key={key}>
										{key}: {value}
									</span>
