...
function toLongLabel(periodYyyyMmDd: string): string {
  ...
  const d = parts.length === 3 && !parts.some(Number.isNaN)
...
function buildNextMonthForecast(
  ...
  if (expBasis != null && expBasis > 0) {
    ...
    const gExp =
      pe != null && pe > 0 ? monthOverMonthGrowth(expBasis, pe, 0.92, 1.08) : growthRev;
    expF = expBasis * gExp;
  ...
  const marginPct = revenueForecast > 0 ? (resultForecast / revenueForecast) * 100 : 0;
...
export async function getKpiPageData(
  ...
  const prevMonthPeriod  = _mk(new Date(_now.getFullYear(), _now.getMonth() - 1, 1));
  const prev2MonthPeriod = _mk(new Date(_now.getFullYear(), _now.getMonth() - 2, 1));
  const prev3MonthPeriod = _mk(new Date(_now.getFullYear(), _now.getMonth() - 3, 1));

  // Fetch values for current month + 3 prior months in one query; resolve kpiDataPeriod after
  const fetchPeriodIds = [currentMonthPeriod, prevMonthPeriod, prev2MonthPeriod, prev3MonthPeriod];

  const [defsRes, valuesRes, dashboardRes, salesHistoryRes] =
    await Promise.all([
  ...
  if (dashboardRes.error) throw new Error(`Dashboard payload load failed: ${dashboardRes.error.message}`);
  if (salesHistoryRes.error) throw new Error(`Sales history load failed: ${salesHistoryRes.error.message}`);
  ...
  // no_show_rate and present_conversion_rate: computed from SM payload funnel
  {
    ...
    if (f) {
      ...
      if (scheduled > 0) {
        current["no_show_rate"] = Math.round((1 - present / scheduled) * 100 * 10) / 10;
      }
      if (present > 0) {
        current["present_conversion_rate"] = Math.round((closings / present) * 100 * 10) / 10;
      }
    }
  }
  ...
  {
    const margem = (current["revenue_total"] ?? 0) - (current["expenses_total"] ?? 0);
  ...
  const projLabels = projectionMonthStarLabels(kpiDataPeriod, projScaled.length);
  ...
  return {
    ...
    previousPeriodLabel: previousPeriod ? toLongLabel(previousPeriod) : undefined,
    previousPreviousPeriodLabel: thirdPeriod ? toLongLabel(thirdPeriod) : undefined,
    current,
    previous,
    previousPrevious,
    currentMeta,
